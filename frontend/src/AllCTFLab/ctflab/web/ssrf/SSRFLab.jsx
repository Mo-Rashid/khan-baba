...
const LABS = [
  {
    ...
    points: 100,
    description:
      "The application fetches a user-supplied URL and returns the response without restriction.",
    objective:
      "Force the server to request an internal resource (localhost / 127.0.0.1).",
    hint: "Try http://127.0.0.1 or http://localhost in the URL parameter.",
    source: `const response = await fetch(userProvidedUrl);
return response.text();`,
  ...
  },
  {
    ...
    points: 150,
    description:
      "The server can reach private IP ranges on the internal network.",
    objective:
      "Access an internal host (e.g. 192.168.x.x or 10.x.x.x).",
    hint: "Try common private ranges: 192.168.0.1, 10.0.0.1, 172.16.0.1",
    source: `// No allowlist — any URL is fetched
fetch(req.query.url);`,
    placeholder: "http://",
  },
  {
    ...
    points: 200,
    description:
      "SSRF can reach the cloud instance metadata service and steal credentials.",
    objective:
      "Query the cloud metadata endpoint (169.254.169.254).",
    hint: "Target http://169.254.169.254/latest/meta-data/ or equivalent.",
    source: `// IMDSv1 reachable
fetch("http://169.254.169.254/...");`,
    placeholder: "http://169.",
  },
  {
    id: 4,
    ...
    points: 250,
    description:
      "The application allows non-HTTP schemes that can be abused (file://, gopher://, dict://).",
    objective:
      "Use an alternative protocol to read local files or interact with internal services.",
    hint: "Try file:///etc/passwd or gopher:// / dict:// payloads.",
    source: `// Scheme not restricted to http/https
url = req.query.url;`,
    placeholder: "file:///etc",
  },
  {
    id: 5,
    ...
    points: 300,
    description:
      "A blacklist blocks 127.0.0.1 and localhost but can be bypassed.",
    objective:
      "Bypass the filter and still reach the internal service.",
    hint: "Try decimal IP, hex, 0.0.0.0, [::1], or DNS rebinding style tricks.",
    source: `if (url.includes("127.0.0.1") || url.includes("localhost")) {
  throw new Error("Blocked");
}`,
    placeholder: "0x7f000001 ",
  },
];
...
const SSRFLab = () => {
  ...
  const [completedLabs, setCompletedLabs] = useState(() => {
    try {
      const saved = localStorage.getItem("khansploit-ssrf-progress");
  ...
  useEffect(() => {
    localStorage.setItem("khansploit-ssrf-progress", JSON.stringify(completedLabs));
  }, [completedLabs]);
  ...
  const submitToBackend = async () => {
    if (!input.trim()) {
      setChallengeMessage("Enter a URL or SSRF payload.");
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/web/ssrf/${currentLab}/submit`, {
        method: "POST",
      ...
      if (data.success) {
        setResult("SSRF successfully triggered!");
        setFlag(data.flag);
        setShowFlag(true);
        setShowSuccess(true);
        ...
        setResult("Payload did not trigger SSRF.");
        setChallengeMessage(data.message || "Try a different URL or technique.");
      }
    } catch (err) {
      console.error(err);
  ...
  const resetProgress = () => {
    localStorage.removeItem("khansploit-ssrf-progress");
    setCompletedLabs([]);
    setCurrentLab(1);
    setInput("");
    setResult("");
  ...
  return (
    <div className="ssrf-lab-page">
      ...
      <section className="ssrf-hero">
        ...
        <p>
          Master Server-Side Request Forgery through progressive hands-on challenges
          covering localhost, internal network, cloud metadata, protocols and filter bypass.
        </p>
      ...
      <main className="lab-layout">
        <aside className="lab-sidebar">
          <div className="sidebar-card">
            <div className="sidebar-title">SSRF CHALLENGES</div>
            {LABS.map((item) => {
          ...
          {!labStarted && !isCompleted ? (
            <div className="start-card">
              ...
              <p>
                ...
                URL fetch feature designed for SSRF training.
              </p>
              ...
              <div className="ctf-card">
                ...
                <div className="browser">
                  ...
                  <div className="browser-content">
                    ...
                    <h3>
                      {currentLab === 1 && "URL Fetcher"}
                      {currentLab === 2 && "Internal Proxy"}
                      {currentLab === 3 && "Preview Service"}
                      {currentLab === 4 && "Resource Loader"}
                      {currentLab === 5 && "Filtered Fetcher"}
                    </h3>
                    <p>Enter the URL you want the server to fetch.</p>
