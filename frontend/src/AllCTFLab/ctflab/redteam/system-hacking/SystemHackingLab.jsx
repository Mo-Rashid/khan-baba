...
const LABS = [
  {
    ...
    difficulty: "Beginner",
    points: 100,
    description: "Gather basic system information: users, OS, processes, and identity.",
    objective: "Submit an enumeration-style finding (whoami, systeminfo, uname, users).",
    hint: "Keywords: enum, whoami, id, systeminfo, uname, process, user.",
    source: `// whoami / id / systeminfo / uname -a`,
    placeholder: "Example: whoami enum systeminfo",
  },
  {
    id: 2,
    ...
    difficulty: "Beginner",
    points: 150,
    description: "Identify weak, default, or reused credentials on the system.",
    objective: "Submit a weak-credential finding or technique answer.",
    hint: "password, default, weak, credential, admin:admin, reuse.",
    source: `// Check default passwords / credential reuse`,
    placeholder: "Example: default weak password ",
  },
  {
    id: 3,
    ...
    points: 200,
    description: "Abuse sudo rights or NOPASSWD entries for higher privileges.",
    objective: "Submit a sudo/privilege path finding.",
    hint: "sudo, sudoers, nopasswd, privilege, gtfo.",
    source: `// sudo -l\n// NOPASSWD entries`,
    placeholder: "Example: sudo ",
  },
  {
    id: 4,
    ...
    difficulty: "Intermediate",
    points: 250,
    description: "Find writable scripts or weak cron/timers that can escalate privileges.",
    objective: "Submit a cron/scheduled-task abuse answer.",
    hint: "cron, crontab, scheduled, writable, timer.",
    source: `// crontab -l\n// /etc/cron* writable scripts`,
    placeholder: "Example: cron",
  },
  {
    id: 5,
    title: "Root Access Path",
    type: "ROOT",
    difficulty: "Advanced",
    points: 300,
    description: "Combine findings into a full path toward root-level access (training).",
    objective: "Submit a root/privesc completion answer.",
    hint: "root, uid=0, privilege escalation, privesc, full compromise.",
    source: `// Final privesc to root (simulated training)`,
    placeholder: "Example: root privilege ",
  },
];

const SystemHackingLab = () => {
  const [currentLab, setCurrentLab] = useState(1);
  const [completedLabs, setCompletedLabs] = useState(() => {
    try {
      const saved = localStorage.getItem("khansploit-sys-hack-progress");
  ...
  useEffect(() => {
    localStorage.setItem("khansploit-sys-hack-progress", JSON.stringify(completedLabs));
  }, [completedLabs]);
  ...
  const submitToBackend = async () => {
    if (!input.trim()) {
      setChallengeMessage("Enter your system hacking finding or technique.");
      return;
    }
    ...
    try {
      const res = await fetch(
        `${API_BASE}/redteam/system-hacking/${currentLab}/submit`,
        {
          method: "POST",
  ...
  const resetProgress = () => {
    localStorage.removeItem("khansploit-sys-hack-progress");
    setCompletedLabs([]);
    setCurrentLab(1);
    setInput("");
    setResult("");
  ...
  // ===== EXPERIENCE SHARE =====
    ...
    const LAB_KEY = "system-hacking"; // change per lab:
    ...
    <div className="xss-lab-page">
      ...
      <section className="xss-hero">
        ...
        <p>
          ...
          cron abuse, and privilege escalation paths.
        </p>
      ...
      <main className="lab-layout">
        <aside className="lab-sidebar">
          <div className="sidebar-card">
            <div className="sidebar-title">SYSTEM HACKING</div>
            {LABS.map((item) => {
        ...
        <section className="challenge-area">
          ...
          {!labStarted && !isCompleted ? (
            <div className="start-card">
              ...
              <p>Host-based privilege path training — technique answers only.</p>
            ...
            <>
              <div className="ctf-card">
                ...
                <div className="browser">
                  <div className="browser-top">
                    ...
                    <div className="browser-address">lab.system-hack.khansploit.local</div>
                    <span>⟳</span>
                  </div>
                  <div className="browser-content">
                    <div className="target-brand">KHAN<span>HOST</span></div>
                    <h3>
                      {currentLab === 1 && "Enumeration Desk"}
                      {currentLab === 2 && "Credential Check"}
                      {currentLab === 3 && "Sudo Audit"}
                      {currentLab === 4 && "Cron Review"}
                      {currentLab === 5 && "Root Path"}
                    </h3>
              ...
              {challengeMessage && (
                <div className="challenge-message"><span>!</span>{challengeMessage}</div>
              )}

              <div className="ctf-card hint-card">
                <button className="collapsible-header" onClick={() => setShowHint(!showHint)}>
                  <div><span className="card-icon">💡</span><strong>Need a Hint?</strong></div>
                  <span>{showHint ? "−" : "+"}</span>
                </button>
                {showHint && (
              ...
              <div className="ctf-card source-card">
                <button className="collapsible-header" onClick={() => setShowSource(!showSource)}>
                  <div><span className="card-icon">{"</>"}</span><strong>Reference Notes</strong></div>
        ...
        <div className="success-overlay">
          <div className="success-modal">
            ...
            <div className="modal-actions">
              <button className="close-modal" onClick={() => setShowSuccess(false)}>Continue Exploring</button>
              {currentLab < TOTAL_LABS && (
                <button className="modal-next" onClick={() => { setShowSuccess(false); nextLab(); }}>
                  Next Challenge →
                </button>
              )}
            </div>
...
export default SystemHackingLab;